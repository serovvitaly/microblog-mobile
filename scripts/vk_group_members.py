...
def get_members_by_group_id(group_id, callback = None):
    ...
    request_list.aggregating_items = False
    request_list.callback = callback
    request_list.exec()
    items = request_list.get_items()
    return items


def callback_items(members):
    if members is None:
        exit()
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        ...
        exit()
    members = get_members_by_group_id(sys.argv[1], callback_items)
